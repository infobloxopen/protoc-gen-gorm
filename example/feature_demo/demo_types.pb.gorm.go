...
type TypeWithIDORM struct {
	...
	NestedMess    *TypeWithID_InternalMsgORM `gorm:"foreignkey:TypeWithIDId;association_foreignkey:Id"`
...
func (m *TypeWithID) ToORM(ctx context.Context) (TypeWithIDORM, error) {
	...
	if m.NestedMess != nil {
		tempNestedMess, err := m.NestedMess.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.NestedMess = &tempNestedMess
	}
...
func (m *TypeWithIDORM) ToPB(ctx context.Context) (TypeWithID, error) {
	...
	if m.NestedMess != nil {
		tempNestedMess, err := m.NestedMess.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.NestedMess = &tempNestedMess
...
type TypeWithID_InternalMsgORM struct {
	F1           string
	TypeWithIDId *uint32
}

// TableName overrides the default tablename generated by GORM
func (TypeWithID_InternalMsgORM) TableName() string {
	return "internal_msgs"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *TypeWithID_InternalMsg) ToORM(ctx context.Context) (TypeWithID_InternalMsgORM, error) {
	to := TypeWithID_InternalMsgORM{}
	var err error
	if prehook, ok := interface{}(m).(TypeWithID_InternalMsgWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	to.F1 = TypeWithID_InternalMsgInternalEnum_name[int32(m.F1)]
	if posthook, ok := interface{}(m).(TypeWithID_InternalMsgWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *TypeWithID_InternalMsgORM) ToPB(ctx context.Context) (TypeWithID_InternalMsg, error) {
	to := TypeWithID_InternalMsg{}
	var err error
	if prehook, ok := interface{}(m).(TypeWithID_InternalMsgWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	to.F1 = TypeWithID_InternalMsgInternalEnum(TypeWithID_InternalMsgInternalEnum_value[m.F1])
	if posthook, ok := interface{}(m).(TypeWithID_InternalMsgWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type TypeWithID_InternalMsg the arg will be the target, the caller the one being converted from

// TypeWithID_InternalMsgBeforeToORM called before default ToORM code
type TypeWithID_InternalMsgWithBeforeToORM interface {
	BeforeToORM(context.Context, *TypeWithID_InternalMsgORM) error
}

// TypeWithID_InternalMsgAfterToORM called after default ToORM code
type TypeWithID_InternalMsgWithAfterToORM interface {
	AfterToORM(context.Context, *TypeWithID_InternalMsgORM) error
}

// TypeWithID_InternalMsgBeforeToPB called before default ToPB code
type TypeWithID_InternalMsgWithBeforeToPB interface {
	BeforeToPB(context.Context, *TypeWithID_InternalMsg) error
}

// TypeWithID_InternalMsgAfterToPB called after default ToPB code
type TypeWithID_InternalMsgWithAfterToPB interface {
	AfterToPB(context.Context, *TypeWithID_InternalMsg) error
}
...
func DefaultReadTypeWithID(ctx context.Context, in *TypeWithID, db *gorm1.DB) (*TypeWithID, error) {
	...
	db = db.Preload("ANestedObject").Preload("NestedMess").Preload("Point").Preload("Things").Preload("User")
...
func DefaultStrictUpdateTypeWithID(ctx context.Context, in *TypeWithID, db *gorm1.DB) (*TypeWithID, error) {
	...
	*filterANestedObject.ANestedObjectTypeWithIDId = ormObj.Id
	...
	filterNestedMess := TypeWithID_InternalMsgORM{}
	if ormObj.Id == 0 {
		return nil, errors.New("Can't do overwriting update with no Id value for TypeWithIDORM")
	}
	filterNestedMess.TypeWithIDId = new(uint32)
	*filterNestedMess.TypeWithIDId = ormObj.Id
	if err = db.Where(filterNestedMess).Delete(TypeWithID_InternalMsgORM{}).Error; err != nil {
		return nil, err
	}
...
// DefaultCreateTypeWithID_InternalMsg executes a basic gorm create call
func DefaultCreateTypeWithID_InternalMsg(ctx context.Context, in *TypeWithID_InternalMsg, db *gorm1.DB) (*TypeWithID_InternalMsg, error) {
	if in == nil {
		return nil, errors.New("Nil argument to DefaultCreateTypeWithID_InternalMsg")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
...
// DefaultListTypeWithID_InternalMsg executes a gorm list call
func DefaultListTypeWithID_InternalMsg(ctx context.Context, db *gorm1.DB, req interface{}) ([]*TypeWithID_InternalMsg, error) {
	ormResponse := []TypeWithID_InternalMsgORM{}
	f, s, p, fs, err := getCollectionOperators(req)
	if err != nil {
		return nil, err
	}
	...
	pbResponse := []*TypeWithID_InternalMsg{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
