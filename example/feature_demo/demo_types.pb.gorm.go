...
type TestAssocHandlerORM struct {
	Id               string
	TestReplaceAssoc []*TestTagAssociationORM `gorm:"foreignkey:TestAssocHandlerId;association_foreignkey:Id;preload:false;replace:true"`
}

// TableName overrides the default tablename generated by GORM
func (TestAssocHandlerORM) TableName() string {
	return "test_assoc_handlers"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *TestAssocHandler) ToORM(ctx context.Context) (TestAssocHandlerORM, error) {
	to := TestAssocHandlerORM{}
	var err error
	if prehook, ok := interface{}(m).(TestAssocHandlerWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(TestAssocHandlerWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *TestAssocHandlerORM) ToPB(ctx context.Context) (TestAssocHandler, error) {
	to := TestAssocHandler{}
	var err error
	if prehook, ok := interface{}(m).(TestAssocHandlerWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(TestAssocHandlerWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type TestAssocHandler the arg will be the target, the caller the one being converted from

// TestAssocHandlerBeforeToORM called before default ToORM code
type TestAssocHandlerWithBeforeToORM interface {
	BeforeToORM(context.Context, *TestAssocHandlerORM) error
}

// TestAssocHandlerAfterToORM called after default ToORM code
type TestAssocHandlerWithAfterToORM interface {
	AfterToORM(context.Context, *TestAssocHandlerORM) error
}

// TestAssocHandlerBeforeToPB called before default ToPB code
type TestAssocHandlerWithBeforeToPB interface {
	BeforeToPB(context.Context, *TestAssocHandler) error
}

// TestAssocHandlerAfterToPB called after default ToPB code
type TestAssocHandlerWithAfterToPB interface {
	AfterToPB(context.Context, *TestAssocHandler) error
}

type TestTagAssociationORM struct {
	SomeField          string
	TestAssocHandlerId *string
	TestTagId          *string
}
...
func DefaultStrictUpdateTypeWithID(ctx context.Context, in *TypeWithID, db *gorm1.DB) (*TypeWithID, error) {
	...
	if err = db.Model(&ormObj).Association("ANestedObject").Clear().Error; err != nil {
		return nil, err
	}
	if err = db.Model(&ormObj).Association("Things").Clear().Error; err != nil {
		return nil, err
	}
...
func DefaultStrictUpdatePrimaryUUIDType(ctx context.Context, in *PrimaryUUIDType, db *gorm1.DB) (*PrimaryUUIDType, error) {
	...
	if err = db.Model(&ormObj).Association("Child").Clear().Error; err != nil {
		return nil, err
	}
...
// DefaultCreateTestAssocHandler executes a basic gorm create call
func DefaultCreateTestAssocHandler(ctx context.Context, in *TestAssocHandler, db *gorm1.DB) (*TestAssocHandler, error) {
	if in == nil {
		return nil, errors1.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TestAssocHandlerORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm1.DB) error
}

// DefaultReadTestAssocHandler executes a basic gorm read call
func DefaultReadTestAssocHandler(ctx context.Context, in *TestAssocHandler, db *gorm1.DB) (*TestAssocHandler, error) {
	if in == nil {
		return nil, errors1.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm2.ApplyFieldSelection(ctx, db, nil, &TestAssocHandlerORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := TestAssocHandlerORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(TestAssocHandlerORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TestAssocHandlerORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm1.DB) error
}

func DefaultDeleteTestAssocHandler(ctx context.Context, in *TestAssocHandler, db *gorm1.DB) error {
	if in == nil {
		return errors1.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&TestAssocHandlerORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type TestAssocHandlerORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm1.DB) error
}

func DefaultDeleteTestAssocHandlerSet(ctx context.Context, in []*TestAssocHandler, db *gorm1.DB) error {
	if in == nil {
		return errors1.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&TestAssocHandlerORM{})).(TestAssocHandlerORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("id in (?)", keys).Delete(&TestAssocHandlerORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&TestAssocHandlerORM{})).(TestAssocHandlerORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type TestAssocHandlerORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*TestAssocHandler, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*TestAssocHandler, *gorm1.DB) error
}

// DefaultStrictUpdateTestAssocHandler clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateTestAssocHandler(ctx context.Context, in *TestAssocHandler, db *gorm1.DB) (*TestAssocHandler, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateTestAssocHandler")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	...
	var count int64
	lockedRow := &TestAssocHandlerORM{}
	count = db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("id=?", ormObj.Id).First(lockedRow).RowsAffected
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type TestAssocHandlerORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm1.DB) error
}

// DefaultPatchTestAssocHandler executes a basic gorm update call with patch behavior
func DefaultPatchTestAssocHandler(ctx context.Context, in *TestAssocHandler, updateMask *field_mask1.FieldMask, db *gorm1.DB) (*TestAssocHandler, error) {
	if in == nil {
		return nil, errors1.NilArgumentError
	}
	var pbObj TestAssocHandler
	var err error
	if hook, ok := interface{}(&pbObj).(TestAssocHandlerWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbReadRes, err := DefaultReadTestAssocHandler(ctx, &TestAssocHandler{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	pbObj = *pbReadRes
	if hook, ok := interface{}(&pbObj).(TestAssocHandlerWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskTestAssocHandler(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(TestAssocHandlerWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateTestAssocHandler(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(TestAssocHandlerWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type TestAssocHandlerWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *TestAssocHandler, *field_mask1.FieldMask, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *TestAssocHandler, *field_mask1.FieldMask, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *TestAssocHandler, *field_mask1.FieldMask, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *TestAssocHandler, *field_mask1.FieldMask, *gorm1.DB) error
}

// DefaultPatchSetTestAssocHandler executes a bulk gorm update call with patch behavior
func DefaultPatchSetTestAssocHandler(ctx context.Context, objects []*TestAssocHandler, updateMasks []*field_mask1.FieldMask, db *gorm1.DB) ([]*TestAssocHandler, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors1.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*TestAssocHandler, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchTestAssocHandler(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskTestAssocHandler patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskTestAssocHandler(ctx context.Context, patchee *TestAssocHandler, patcher *TestAssocHandler, updateMask *field_mask1.FieldMask, prefix string, db *gorm1.DB) (*TestAssocHandler, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors1.NilArgumentError
...
// DefaultListTestAssocHandler executes a gorm list call
func DefaultListTestAssocHandler(ctx context.Context, db *gorm1.DB) ([]*TestAssocHandler, error) {
	in := TestAssocHandler{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm2.ApplyCollectionOperators(ctx, db, &TestAssocHandlerORM{}, &TestAssocHandler{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(TestAssocHandlerORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*TestAssocHandler{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type TestAssocHandlerORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm1.DB) (*gorm1.DB, error)
}
type TestAssocHandlerORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm1.DB, *[]TestAssocHandlerORM) error
}
