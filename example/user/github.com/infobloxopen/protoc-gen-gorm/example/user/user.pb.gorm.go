...
import (
	...
	stdStringsImport "stdStringsImport"
	time "time"
)
...
func (m *User) ToORM(ctx context.Context) (UserORM, error) {
	...
	to.Num = m.Num
	// Repeated type message is not an ORMable message type
	// Repeated type message is not an ORMable message type
	// Repeated type message is not an ORMable message type
	// Repeated type message is not an ORMable message type
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedBillingAddress && strings.HasPrefix(f, prefix+"BillingAddress.") {
			...
			if patchee.BillingAddress == nil {
				patchee.BillingAddress = &message{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"BillingAddress."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.BillingAddress, patchee.BillingAddress, childMask); err != nil {
				return nil, nil
			}
		}
		...
		if !updatedShippingAddress && strings.HasPrefix(f, prefix+"ShippingAddress.") {
			...
			if patchee.ShippingAddress == nil {
				patchee.ShippingAddress = &message{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"ShippingAddress."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.ShippingAddress, patchee.ShippingAddress, childMask); err != nil {
				return nil, nil
			}
		}
		...
		if !updatedShippingAddressId && strings.HasPrefix(f, prefix+"ShippingAddressId.") {
			...
			if patchee.ShippingAddressId == nil {
				patchee.ShippingAddressId = &message{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
...
func DefaultDeleteEmailSet(ctx context.Context, in []*Email, db *gorm.DB) error {
	...
	err = db.Where("id in (?)", keys).Delete(&EmailORM{}).Error
	if err != nil {
		return err
	}
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedExternalNotNull && strings.HasPrefix(f, prefix+"ExternalNotNull.") {
			...
			if patchee.ExternalNotNull == nil {
				patchee.ExternalNotNull = &message{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
