...
type UserORM struct {
	Birthday   time.Time
	CreatedAt  time.Time
	CreditCard *CreditCardORM `gorm:"foreignkey:CreditCardUserId;association_foreignkey:Id"`
	Emails     []*EmailORM    `gorm:"foreignkey:EmailsUserId;association_foreignkey:Id"`
	Id         int32
	Num        uint32
	Tasks      []*TaskORM `gorm:"foreignkey:TasksUserId;association_foreignkey:Id"`
	UpdatedAt  time.Time
}
...
func DefaultStrictUpdateUser(ctx context.Context, in *User, db *gorm.DB) (*User, error) {
	...
	filterCreditCard.CreditCardUserId = ormObj.Id
	...
	filterEmails.EmailsUserId = ormObj.Id
	...
	filterTasks.TasksUserId = ormObj.Id
